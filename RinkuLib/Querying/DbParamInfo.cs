...
/// <summary>The default parameter-binding services used when no provider-specific registration claims a parameter.</summary>
public interface IDbParameterDefaults {
    /// <summary>The unsettled strategy used before provider metadata has been learned.</summary>
    DbParamInfo Inferred { get; }
    ...
    public static bool TryGetParamInfo(IDbCommand cmd, string paramName, [MaybeNullWhen(false)] out DbParamInfo param) {
        ...
        for (int i = 0; i < count; i++) {
            if (parameters[i] is not IDbDataParameter pa || pa.ParameterName != paramName)
                continue;
            param = DbParameterDefaults.Current.MakeInfo(pa);
            return true;
        }
        param = null;
        return false;
