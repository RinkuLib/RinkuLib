...
/// <summary>Creates the metadata implementations used when the registry has no explicit entry.</summary>
public interface ITypeParsingInfoFactory {
    /// <summary>The metadata used for scalar database values.</summary>
    TypeParsingInfo Scalar { get; }
    /// <summary>The metadata used for one-dimensional arrays.</summary>
    TypeParsingInfo Array { get; }
    /// <summary>Creates metadata for an object or open generic type.</summary>
    TypeParsingInfo Create(Type type);
}
/// <summary>
...
public abstract class TypeParsingInfo {
    internal static void ApplyRegistrationInitializer<T>(Action<T>? initializer, T item) {
        initializer?.Invoke(item);
    }
    private static void ApplyRegistrationInitializer<T1, T2>(Action<T1, T2>? initializer, T1 first, T2 second) {
        initializer?.Invoke(first, second);
    }
    ...
    /// <summary>Identify if the instance can actually handle the <see cref="Type"/> of <paramref name="TargetType"/></summary>
    public abstract void ValidateCanUseType(Type TargetType);
    private static ITypeParsingInfoFactory? _defaultFactory;
    private static Action<Type, TypeParsingInfo>? _registrationInitializer;
    /// <summary>
    /// Adjusts metadata created by <see cref="DefaultFactory"/> before the registry can publish it. The
    /// requested type is supplied separately so the callback can stay implementation-neutral. Configure this
    /// once during application startup. Explicit metadata passed to <see cref="GetOrAdd(Type, TypeParsingInfo?, bool)"/>
    /// or <see cref="AddOrSet(Type, TypeParsingInfo)"/> bypasses it.
    /// </summary>
    public static Action<Type, TypeParsingInfo>? RegistrationInitializer {
        get => Volatile.Read(ref _registrationInitializer);
        set => Volatile.Write(ref _registrationInitializer, value);
    }
    /// <summary>The factory used when lookup needs to create metadata.</summary>
    public static ITypeParsingInfoFactory DefaultFactory {
        get => _defaultFactory ?? throw new RinkuInternalException(ErrorCodes.InternalInvariant, "No default type-metadata factory has been installed");
        set => _defaultFactory = value ?? throw new ArgumentNullException(nameof(value));
    }
    /// <summary>
    /// Installs an initial metadata factory once. The shipped implementation uses this during bootstrap
    /// without coupling the registry to that implementation.
    /// </summary>
    public static bool TryInstallDefaultFactory(ITypeParsingInfoFactory factory)
        => Interlocked.CompareExchange(ref _defaultFactory, factory ?? throw new ArgumentNullException(nameof(factory)), null) is null;
    private static TypeParsingInfo CreateDefault(Type type) {
        var info = DefaultFactory.Create(type);
        Volatile.Read(ref _registrationInitializer)?.Invoke(type, info);
        return info;
    }
    /// <summary>
    ...
    public static bool TryGetInfo(Type type, [MaybeNullWhen(false)] out TypeParsingInfo typeInfo) {
        ...
        if (type.IsBaseType() || type.IsEnum) {
            typeInfo = TypeInfos.GetOrAdd(type, DefaultFactory.Scalar);
            return true;
        }
        if (type.IsSZArray) {
            typeInfo = TypeInfos.GetOrAdd(type, DefaultFactory.Array);
            return true;
        }
    ...
    public static TypeParsingInfo ForceGet(Type type) {
        ...
        if (TypeInfos.TryGetValue(type, out var infos))
            return infos;
        if (!type.IsGenericType) {
            infos = type.IsBaseType() || type.IsEnum ? DefaultFactory.Scalar
                : type.IsSZArray ? DefaultFactory.Array
                : CreateDefault(type);
    ...
    public static TypeParsingInfo GetOrAdd(Type type, TypeParsingInfo? toUseIfNotPresent = null, bool saveAsGenericDefinitionWhenGeneric = true) {
        ...
        if (!type.IsGenericType || !saveAsGenericDefinitionWhenGeneric) {
            ...
            infos = toUseIfNotPresent ?? (type.IsBaseType() || type.IsEnum
                ? DefaultFactory.Scalar : CreateDefault(type));
    ...
    /// </summary>
    /// <remarks>This changes registration state only. It does not inspect, invalidate, notify, or dispose parsers already held by any cache.</remarks>
    ...
    /// <summary>
    /// Removes the metadata registered under exactly <paramref name="type"/>. This does not normalize nullable
    /// or generic types and does not invalidate parsers that were already generated from the metadata.
    /// </summary>
    /// <remarks>
    /// The returned metadata remains an ordinary live object for callers that still hold it. Future mapping
    /// negotiations no longer find it through this exact registry key.
    /// </remarks>
