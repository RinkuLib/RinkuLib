...
public interface ITypeParser : IDisposable {
    /// <summary>Releases resources owned directly by this parser. Implementations with no owned resources need no override.</summary>
    /// <remarks>
    /// Disposal must be idempotent. A wrapper does not own a child parser supplied by a maker and therefore
    /// must not dispose that child; the parser cache coordinates shared parser lifetimes.
    /// </remarks>
    void IDisposable.Dispose() { }
    /// <summary>Whether this parser can read a result carrying <paramref name="schema"/>.</summary>
    /// <remarks>
    /// A parser owns its compatibility rule. Generated parsers normally compare against the shape they were
    /// built from, while a schema-independent parser may return <see langword="true"/> for every shape.
    /// This is consulted when selecting or merging cached parsers, never while parsing rows.
    /// </remarks>
