...
A `QueryCommand` is a reusable SQL template. Execution methods sit directly on it.
...
The available result shapes are covered on [result shapes](result-shapes.md).
...
```csharp
static readonly QueryCommand ByComposer = new("SELECT TrackId AS Id, Name, UnitPrice FROM tracks WHERE Composer = @composer AND UnitPrice >= @minPrice");
...
```csharp
static readonly QueryCommand UpdatePrice = new("UPDATE tracks SET UnitPrice = @price WHERE TrackId = @id");
static readonly QueryCommand CountTracks = new("SELECT COUNT(*) FROM tracks");
static readonly QueryCommand InsertTrack = new("INSERT INTO tracks (Name) VALUES (@name) RETURNING TrackId");
...
```csharp
static readonly QueryCommand UpdatePlaylist = new("UPDATE playlists SET Name = @name, IsPublic = @isPublic WHERE PlaylistId = @playlistId");
...
```csharp
static readonly QueryCommand UpdateTrack = new("UPDATE tracks SET Name = @name WHERE TrackId = @id", ["name", "id"], CommandType.Text);
```
...
```csharp
static readonly QueryCommand Tracks = new("SELECT TrackId, Name FROM tracks WHERE AlbumId = ?@albumId");
...
```csharp
static readonly QueryCommand Invoices = new("SELECT i.InvoiceId FROM invoices i /*@country*/INNER JOIN customers c ON i.CustomerId = c.CustomerId WHERE c.Country = ?@country");
...
```csharp
static readonly QueryCommand TracksByGenre = new("SELECT TrackId AS Id, Name, UnitPrice FROM tracks WHERE GenreId IN (?@genreIds_X)");
...
```csharp
static readonly QueryCommand TrackColumns = new("?SELECT TrackId AS Id!, Name, UnitPrice FROM tracks WHERE AlbumId = @albumId");
...
`Query<T>` uses `T` to choose the result shape. The async `QueryAsync<T>` form returns `Task<T>` for the same shapes:

| Shape | Use |
| --- | --- |
| One row | `T` |
| One row or empty | `Optional<T>` or `OptionalStruct<T>` |
| Exactly one row | `Single<T>` |
| Buffered rows | `List<T>` |
| Synchronous stream | `IEnumerable<T>` |

For an async stream, use `StreamQueryAsync<T>`, which returns `IAsyncEnumerable<T>` directly.

Other execution surfaces:

| Need | Sync | Async |
| --- | --- | --- |
| Insert, update, or delete | `Execute` → `int` | `ExecuteAsync` → `Task<int>` |
| One value from execution | `ExecuteScalar<T>` → `T` | `ExecuteScalarAsync<T>` → `Task<T>` |
| Raw reader | `ExecuteReader` → `DbDataReader` | `ExecuteReaderAsync` → `Task<DbDataReader>` |
| Several result sets | `ExecuteMultiReader` → `MultiReader` | `ExecuteMultiReaderAsync` → `Task<MultiReader>` |
