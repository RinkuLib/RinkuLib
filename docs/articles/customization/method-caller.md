...
int affected = await save(
    new SaveAlbumArgs(12, "Blue"),
    cancellationToken);
```
...
```csharp
Func<SaveAlbumArgs, int, Task<int>> save =
    MethodCaller.Create<Func<SaveAlbumArgs, int, Task<int>>>(
        method,
...
```csharp
Func<SaveAlbumArgs, int, CancellationToken, Task<int>> save =
    MethodCaller.Create<Func<SaveAlbumArgs, int, CancellationToken, Task<int>>>(
        method,
        CallerParameter<int>.Named("userId"),
        CallerParameter<CancellationToken>.ByType());
```
...
```csharp
Func<SaveAlbumArgs, Task<int>> save =
    MethodCaller.Create<Func<SaveAlbumArgs, Task<int>>>(method);
```
