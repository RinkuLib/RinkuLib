...
public class CompleteTests {
    ...
    public void Example1_StaticQuery() {
        ...
        using var cnn = GetDbCnn();
        var p = builder.Query<Person>(cnn);
        Assert.NotNull(p);
        Assert.Equal(1, p.ID);
        ...
        var p2 = builder2.Query<Person>(cnn);
        Assert.NotNull(p2);
    ...
    public async Task Example1_StaticQuery_Async() {
        ...
        builder.Use("@Active", true);
        using var cnn = GetDbCnn();
        var p = await builder.QueryAsync<Person>(cnn, ct: TestContext.Current.CancellationToken);
        Assert.NotNull(p);
        Assert.Equal(1, p.ID);
        Assert.Equal("John", p.Username);
        Assert.Null(p.Email);
    ...
    public async Task Example1_StaticQuery_Object() {
        ...
        builder.Use("@Active", true);
        using var cnn = GetDbCnn();
        var (p, email) = builder.Query<(Person, object)>(cnn);
        Assert.Equal(1, p.ID);
        Assert.Equal("John", p.Username);
        Assert.Null(email);
    }
    ...
    public async Task Use_Complete_Obj() {
        var query = new QueryCommand("SELECT ID, Name, Email AS Emaill FROM Users WHERE IsActive = @Active");
        using var cnn = GetDbCnn();
        var (p, email) = query.Query<(Person, object)>(cnn, new PersonParam(true));
        Assert.Equal(1, p.ID);
        Assert.Equal("John", p.Username);
        Assert.Null(email);
    }
    ...
    public async Task Use_Complete_T() {
        var query = new QueryCommand("SELECT ID, Name, Email AS Emaill FROM Users WHERE IsActive = @Active");
        using var cnn = GetDbCnn();
        var (p, email) = query.Query<(Person, object), PersonParam>(cnn, new PersonParam(true));
        Assert.Equal(1, p.ID);
        Assert.Equal("John", p.Username);
        Assert.Null(email);
    }
    ...
    public async Task Use_Complete_T_False() {
        var query = new QueryCommand("SELECT ID, Name, Email AS Emaill FROM Users WHERE IsActive = @Active");
        using var cnn = GetDbCnn();
        var (p, email) = query.Query<(Person, object), PersonParam>(cnn, new PersonParam(false));
        Assert.Equal(2, p.ID);
    ...
    public void Cache_Bug() {
        ...
        var dObj1 = builder1.Query<DynaObject>(cnn);
    ...
    public async Task Using_Default_Nullable() {
        ...
        using var cnn = GetDbCnn();
        var withNullable = query.Query<WithDefNullable>(cnn, new { ID = 1 });
