...
public class RegistrationResidualTests {
    ...
    public sealed class DirectGenericElement {
        public int Value;
        public DirectGenericElement([NoName] int value) => Value = value;
    }

    public sealed class ReadableListHolder : IDbReadable {
        public List<DirectGenericElement> Items;
        [AreReadable]
        public ReadableListHolder([NoName] List<DirectGenericElement> items) => Items = items;
    }

    public sealed class DirectPairLeft {
        public int Value;
        public DirectPairLeft([NoName] int value) => Value = value;
    }

    public sealed class DirectPairRight {
        public int Value;
        public DirectPairRight([NoName] int value) => Value = value;
    }

    public sealed class ReadablePairHolder : IDbReadable {
        public KeyValuePair<DirectPairLeft, DirectPairRight> Pair;
        [AreReadable]
    ...
    [Fact]
    public void AreReadable_lets_the_generic_child_register_its_direct_argument() {
        Assert.Null(TypeParsingInfo.Get(typeof(DirectGenericElement)));

        ColumnInfo[] cols = [new("Value", typeof(int), false)];
        var built = Rows.ParseOne<ReadableListHolder>(cols, 5);

        Assert.Equal(5, Assert.Single(built.Items).Value);
        Assert.NotNull(TypeParsingInfo.Get(typeof(DirectGenericElement)));
    }

    [Fact]
    public void AreReadable_lets_a_constructor_generic_child_register_each_argument() {
        Assert.Null(TypeParsingInfo.Get(typeof(DirectPairLeft)));
        Assert.Null(TypeParsingInfo.Get(typeof(DirectPairRight)));
        TypeParsingInfo.AddOrSet(typeof(KeyValuePair<,>), CtorTypeInfo.Instance);

        ColumnInfo[] cols = [new("key", typeof(int), false), new("value", typeof(int), false)];
        var built = Rows.ParseOne<ReadablePairHolder>(cols, 5, 7);

        Assert.Equal(5, built.Pair.Key.Value);
    ...
    sealed class ReshapableInfo : TypeParsingInfo, ICanUpdateAltNames, ICanUpdateNullColHandlers {
        ...
        public override DbItemPlan? TryGetParser(Type currentClosedType, RecursiveInfo previousUsages,
            ParamInfo paramInfo, ColumnInfo[] columns, ColModifier colModifier, ref ColumnUsage colUsage, MethodCtorInfo.AdditionalFlags callerFlags = default) => null;
    }
