...
public class RegistrationApiTests {
    ...
    public void Flags_come_from_attributes_or_are_passed_explicitly() {
        ...
        viaCtor.Flags = MethodCtorInfo.AdditionalFlags.CanCompleteWithMembers;
        Assert.True(viaCtor.CanCompleteWithMembers);
        Assert.False(viaCtor.ParametersAreReadable);
    }
    ...
    record ConfigurablePath(int Id) : IDbReadable;

    [Fact]
    public void A_construction_handle_exposes_path_settings_together() {
        var info = TypeParsingInfo.GetOrAdd<ConfigurablePath>();
        var path = info.GetConstruction(typeof(int));

        path.GroupKey = new EqualityGroupingRule(["Id"]);
        path.Flags |= MethodCtorInfo.AdditionalFlags.CanCompleteWithMembers;
        path.Parameters[0].UpdateAltName(_ => new NameComparer("ExternalId"));

        Assert.NotNull(path.GroupKey);
        Assert.True(path.CanCompleteWithMembers);
        Assert.True(path.Parameters[0].NameComparer.Contains("ExternalId"));
    }
    ...
    class AttributedMembers {
        ...
        public UnregisteredPlain? UnusableField = null;
    }
