...
public class AltNamesAndScopeTests {
    ...
    [Fact]
    public void Runtime_name_configuration_can_swap_two_members() {
        ColumnInfo[] cols = [new("A", typeof(string), false), new("B", typeof(string), false)];
        var normal = Rows.ParseOne<SwappedMembers>(cols, "AVal", "BVal");
        Assert.Equal("AVal", normal.A);
        Assert.Equal("BVal", normal.B);

        var info = TypeParsingInfo.GetOrAdd<SwappedMembers>();
        Assert.True(info.UpdateAltName(names => names.GetDefaultName() switch {
            "A" => new NameComparer("B"),
            "B" => new NameComparer("A"),
            _ => null,
        }));

        var swapped = Rows.ParseOne<SwappedMembers>(cols, "AVal", "BVal");
        Assert.Equal("BVal", swapped.A);
        Assert.Equal("AVal", swapped.B);
    }
...
public record SwappedMembers(string A, string B) : IDbReadable;
