...
public class TypeConversionTests {
    ...
    [Fact]
    public void Exact_converter_accepts_only_the_same_non_nullable_type() {
        Assert.True(ITypeConverter.TryGetExactConverter(typeof(int), typeof(int), out var identity));
        Assert.IsType<IdentityConverter>(identity);
        Assert.False(ITypeConverter.TryGetExactConverter(typeof(long), typeof(int), out _));
        Assert.True(ITypeConverter.TryGetExactConverter(typeof(int), typeof(int?), out var nullable));
        Assert.IsType<NullableWrapperConverter>(nullable);
    }
    ...
    [Fact]
    public void DateOnly_and_TimeOnly_columns_round_trip() {
        var date = new DateOnly(2024, 5, 1);
        var time = new TimeOnly(13, 30);
        ColumnInfo[] dateCols = [new("Date", typeof(DateOnly), false)];
        ColumnInfo[] timeCols = [new("Time", typeof(TimeOnly), false)];
        Assert.Equal(date, Rows.ParseOne<DateOnly>(dateCols, date));
        Assert.Equal(time, Rows.ParseOne<TimeOnly>(timeCols, time));
    }

    [Fact]
    public void Nullable_DateOnly_and_TimeOnly_columns_accept_null() {
        ColumnInfo[] dateCols = [new("Date", typeof(DateOnly), true)];
        ColumnInfo[] timeCols = [new("Time", typeof(TimeOnly), true)];
        Assert.Null(Rows.ParseOne<DateOnly?>(dateCols, DBNull.Value));
        Assert.Null(Rows.ParseOne<TimeOnly?>(timeCols, DBNull.Value));
    }
