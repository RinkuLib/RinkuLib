...
public class ParserQueryRoadsTests(SqliteDb Db) : IClassFixture<SqliteDb> {
    ...
    static readonly string[] ExpectedNames = ["John", "Victor", "Alice"];
    ...
    public void The_lazy_shape_streams_and_cleans_up_at_the_end() {
        ...
        using var open = Db.Open();
        using var cmd2 = Cmd(open, Names);
        Assert.Equal(ExpectedNames, Parser<IEnumerable<string>>().Query(cmd2));
        Assert.Equal(ConnectionState.Open, open.State);
        ...
        Assert.Equal(ExpectedNames, Parser<IEnumerable<string>>().Query(disposing, disposeCommand: true));
    }
    ...
    public async Task The_lazy_shape_async_wrappers_deliver_the_same_rows() {
        ...
        using var cnn = Db.GetConnection();
        using var cmd = Cmd(cnn, Names);
        Assert.Equal(ExpectedNames, await Parser<IEnumerable<string>>().QueryAsync(cmd, ct: ct));
        using var cmd2 = Cmd(cnn, Names);
        Assert.Equal(ExpectedNames, await Parser<IEnumerable<string>>().QueryAsync(cmd2, cache, ct: ct));
        using var cmd3 = Cmd(cnn, Names);
        Assert.Equal(ExpectedNames, await Parser<IEnumerable<string>>().QueryAsync((IDbCommand)cmd3, ct: ct));
        using var cmd4 = Cmd(cnn, Names);
        Assert.Equal(ExpectedNames, await Parser<IEnumerable<string>>().QueryAsync((IDbCommand)cmd4, cache, ct: ct));
        using var cmd5 = Cmd(cnn, Names);
        IDbCommand plain = new PlainCommand(cmd5);
        Assert.Equal(ExpectedNames, await Parser<IEnumerable<string>>().QueryAsync(plain, ct: ct));
        Assert.Equal(ExpectedNames, await Parser<IEnumerable<string>>().QueryAsync(plain, cache, ct: ct));
    }
    ...
    public async Task The_remaining_shape_and_row_crossings() {
        ...
        using var cmd = Cmd(cnn, Names);
        Assert.Equal(ExpectedNames, Parser<List<string>>().Query(cmd, cache));
        using var cmd2 = Cmd(cnn, Names);
        Assert.Equal(ExpectedNames, Parser<List<string>>().Query((IDbCommand)new PlainCommand(cmd2), cache));
        using var cmd3 = Cmd(cnn, Names);
        ...
        Assert.Equal([new UserRow(1, "John"), new UserRow(2, "Victor"), new UserRow(3, "Alice")], lazyUsers.Query((IDbCommand)new PlainCommand(cmd5), cache));
        using var cmd6 = Cmd(cnn, "SELECT ID, Name FROM Users ORDER BY ID");
        Assert.Equal([new UserRow(1, "John"), new UserRow(2, "Victor"), new UserRow(3, "Alice")], lazyUsers.Query(cmd6, cache));
        using var cmd7 = Cmd(cnn, "SELECT ID, Name FROM Users ORDER BY ID");
        Assert.Equal([new UserRow(1, "John"), new UserRow(2, "Victor"), new UserRow(3, "Alice")], lazyUsers.Query((IDbCommand)new PlainCommand(cmd7)));
    }
