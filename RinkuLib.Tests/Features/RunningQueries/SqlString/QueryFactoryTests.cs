...
public class QueryFactoryTests {
    ...
    [Fact]
    public void Unicode_variable_names_are_parsed_and_bound_as_written() {
        var query = new QueryCommand("SELECT @æøå٦");
        var builder = query.StartBuilder();

        Assert.True(builder.Use("@æøå٦", 42));
        Render.Expect(builder, "SELECT @æøå٦", ("@æøå٦", 42));
    }
