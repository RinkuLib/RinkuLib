...
public class QueryTests(SqliteDb Db) : IClassFixture<SqliteDb> {
    ...
    static readonly QueryCommand ByIds = new("SELECT Name FROM Users WHERE ID IN (@ids_X) ORDER BY ID");
    static readonly QueryCommand ByOptionalIds = new("SELECT Name FROM Users WHERE ID IN (?@ids_X) ORDER BY ID");
    ...
    public void A_spread_run_without_its_collection_is_refused() {
        ...
        Assert.Equal(["John", "Victor"], ByIds.Query<List<string>>(cnn, new { ids = new[] { 1, 2 } }));
        Assert.Equal(["John", "Victor", "Alice"], ByOptionalIds.Query<List<string>>(cnn));
    }
    ...
    public void Query_with_out_command_hands_the_command_back() {
        ...
        Assert.Equal((1L, "John", (string?)null), (user.ID, user.Name, user.Email));
        Assert.NotNull(cmd);
        Assert.Equal("SELECT ID, Name, Email FROM Users ORDER BY ID", cmd.CommandText);
        cmd.Dispose();
    }
