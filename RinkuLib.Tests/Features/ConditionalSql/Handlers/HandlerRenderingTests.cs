...
public class HandlerRenderingTests {
    ...
    [Fact]
    public void Spread_variables_with_shared_prefixes_keep_their_own_values() {
        var query = new QueryCommand("SELECT * FROM t WHERE a IN (?@Id_X) AND b IN (?@Id_Extra_X)");

        Render.Expect(query, new { Id = new[] { 1, 2 }, Id_Extra = new[] { 3, 4 } },
            "SELECT * FROM t WHERE a IN (@Id_1, @Id_2) AND b IN (@Id_Extra_1, @Id_Extra_2)",
            ("@Id_1", 1), ("@Id_2", 2), ("@Id_Extra_1", 3), ("@Id_Extra_2", 4));
    }
