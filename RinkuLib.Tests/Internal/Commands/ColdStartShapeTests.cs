...
public class ColdStartShapeTests(SqliteDb Db) : IClassFixture<SqliteDb> {
    ...
    public async Task The_async_run_does_its_opening_while_it_can_await() {
        ...
        Assert.True(Streamed.TryColdStartAsync<IEnumerable<UserRow>>((DbCommand)cmd, cache, false, TestContext.Current.CancellationToken, out var task));
    ...
    public async Task Leaving_the_async_walk_early_closes_the_reader() {
        ...
        Assert.True(Streamed.TryColdStartAsync<IEnumerable<UserRow>>((DbCommand)cmd, new CountingCache(), false, TestContext.Current.CancellationToken, out var task));
