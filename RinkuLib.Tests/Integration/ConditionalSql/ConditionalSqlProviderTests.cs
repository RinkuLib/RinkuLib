...
public abstract class ConditionalSqlSuite {
    ...
    public async Task A_supplied_spread_filters_on_its_elements() {
        ...
        var ids = await OptionalSpread.QueryAsync<List<int>>(cnn, new { GenreIds = new[] { 1, 3 } }, ct: TestContext.Current.CancellationToken);
    ...
    public async Task An_empty_spread_prunes_into_sql_the_engine_accepts() {
        using var cnn = Connect();
        var all = await OptionalSpread.QueryAsync<List<int>>(cnn, new { GenreIds = Array.Empty<int>() }, ct: TestContext.Current.CancellationToken);
        Assert.Equal([1, 2, 3, 4, 5], all);

        var absent = await OptionalSpread.QueryAsync<List<int>>(cnn, ct: TestContext.Current.CancellationToken);
    ...
    public async Task An_empty_required_spread_never_reaches_the_engine() {
        ...
        await Assert.ThrowsAsync<RequiredHandlerValueException>(
            () => RequiredSpread.QueryAsync<List<int>>(cnn, new { GenreIds = Array.Empty<int>() }, ct: TestContext.Current.CancellationToken));

        var supplied = await RequiredSpread.QueryAsync<List<int>>(cnn, new { GenreIds = new[] { 2 } }, ct: TestContext.Current.CancellationToken);
    ...
    public async Task A_welded_condition_leaves_with_its_partner() {
        using var cnn = Connect();
        Assert.Equal(2, await WeldedSpread.QueryAsync<int>(cnn, new { GenreIds = new[] { 1 } }, ct: TestContext.Current.CancellationToken));
        Assert.Equal(5, await WeldedSpread.QueryAsync<int>(cnn, new { GenreIds = Array.Empty<int>() }, ct: TestContext.Current.CancellationToken));
    }
    ...
    public async Task A_pruned_spread_leaves_the_rest_of_its_list_intact() {
        using var cnn = Connect();
        var both = await SpreadBesideAnother.QueryAsync<List<int>>(cnn, new { Keep = 4, GenreIds = new[] { 1, 2 } }, ct: TestContext.Current.CancellationToken);
        Assert.Equal([1, 2, 4], both);

        var keepOnly = await SpreadBesideAnother.QueryAsync<List<int>>(cnn, new { Keep = 4, GenreIds = Array.Empty<int>() }, ct: TestContext.Current.CancellationToken);
    ...
    [InlineData(null, 1.29, "2,4,5")]
    [InlineData(2, 1.50, "4")]
    public async Task Each_combination_of_optional_filters_runs(int? genreId, double? minPrice, string expectedIds) {
        using var cnn = Connect();
        ...
        var ids = await b.QueryAsync<List<int>>(cnn, ct: TestContext.Current.CancellationToken);
        Assert.Equal(expectedIds.Split(',').Select(int.Parse), ids);
    }
    ...
    public async Task A_bound_command_rebinds_a_spread_across_runs() {
        ...
        Assert.Equal(4, await b.ExecuteScalarAsync<int>(ct: TestContext.Current.CancellationToken));

        b.Use("@GenreIds", new[] { 3 });
        Assert.Equal(1, await b.ExecuteScalarAsync<int>(ct: TestContext.Current.CancellationToken));

        b.Use("@GenreIds", new[] { 1, 2, 3 });
        Assert.Equal(5, await b.ExecuteScalarAsync<int>(ct: TestContext.Current.CancellationToken));
