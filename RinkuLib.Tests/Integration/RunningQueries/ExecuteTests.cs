...
public class ExecuteTests(ExecuteTestsFixture fixture) : IClassFixture<ExecuteTestsFixture> {
    ...
    [Fact]
    public async Task A_procedure_return_value_is_read_from_the_live_command() {
        using var cnn = Fixture.GetConnection();
        var ct = TestContext.Current.CancellationToken;
        await cnn.OpenAsync(ct);

        const string drop = "IF OBJECT_ID('dbo.ReturnAndOutput') IS NOT NULL DROP PROCEDURE dbo.ReturnAndOutput";
        const string create = "CREATE PROCEDURE dbo.ReturnAndOutput @a INT, @moved INT OUTPUT AS BEGIN SET @moved = @a + 1; SELECT @moved AS Total; RETURN 42 END";
        await new QueryCommand(drop).ExecuteAsync(cnn, ct: ct);
