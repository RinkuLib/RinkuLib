...
public abstract class SchemaDiscoverer {
    public abstract Task<DiscoveredSchema> DiscoverSchemaAsync(ExtensionSettings settings, DbConnection cnn, QuerySetting query, CancellationToken ct);
}
public class SchemaDiscoverer<T> : SchemaDiscoverer where T : ISchemaDiscovererStrategy {
    public override async Task<DiscoveredSchema> DiscoverSchemaAsync(ExtensionSettings settings, DbConnection cnn, QuerySetting query, CancellationToken ct) {
        if (cnn.State != ConnectionState.Open)
            await cnn.OpenAsync(ct);
        return await T.DiscoverSchemaAsync(settings, cnn, query, ct);
    }
}
public interface ISchemaDiscovererStrategy {
    abstract static bool CanHandle(DbConnection cnn);
    abstract static Task<DiscoveredSchema> DiscoverSchemaAsync(ExtensionSettings settings, DbConnection cnn, QuerySetting query, CancellationToken ct);
