...
public class SQLServerDiscoveryStrategy : ISchemaDiscovererStrategy {
    ...
    public static bool CanHandle(DbConnection cnn)
        => cnn is SqlConnection;
    public static async Task<DiscoveredSchema> DiscoverSchemaAsync(ExtensionSettings settings, DbConnection cnn, QuerySetting query, CancellationToken ct) {
        ...
        var rawParams = query.SourceType == QuerySourceType.StoredProcedure
            ...
            : await DiscoverInlineQueryParametersAsync(sqlCnn, await settings.GetSqlTextAsync(query, ct), ct);

        var finalParams = ISchemaDiscovererStrategy.ApplyUserOverrides(rawParams, query.Parameters);
        string sqlPayload = query.SourceType == QuerySourceType.StoredProcedure ? $"EXEC {query.Target}" : await settings.GetSqlTextAsync(query, ct);
